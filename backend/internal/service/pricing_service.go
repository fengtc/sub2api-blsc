...
func (s *PricingService) matchByModelFamily(model string) *LiteLLMModelPricing {
	...
	families := []modelFamily{
		{name: "fable-5", match: []string{"claude-fable-5", "claude-fable.5"}},
		{name: "opus-4.8", match: []string{"claude-opus-4-8", "claude-opus-4.8"}, pricing: []string{"claude-opus-4-8", "claude-opus-4.8"}},
		...
		{name: "sonnet-5", match: []string{"claude-sonnet-5", "claude-sonnet.5"}},
	...
	if matched == nil {
		...
		case strings.Contains(model, "sonnet"):
			switch {
			case strings.Contains(model, "sonnet-5") || strings.Contains(model, "sonnet.5"):
