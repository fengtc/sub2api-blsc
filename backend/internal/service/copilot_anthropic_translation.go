...
func translateAnthropicToOpenAI(body []byte, modelMapping map[string]string) ([]byte, error) {
	...
	sanitizeCopilotClaudeToolHistory(&req)
...
// before the request is translated to OpenAI Chat Completions for Copilot.
//
// Copilot's Claude models sit behind an OpenAI-compatible /chat/completions
// facade, but the upstream still validates Claude-style tool history strictly:
// every assistant tool_use must be followed immediately by a user tool_result
// for the same id. Long Claude Code sessions can contain truncated or reordered
// tool history, which Copilot rejects with 400 errors. Downgrading only the
// malformed blocks to text preserves context without inventing tool results.
func sanitizeCopilotClaudeToolHistory(req *AnthropicMessagesRequest) {
	...
	for i := range req.Messages {
		...
		switch msg.Role {
		case "assistant":
			toolUses := anthropicToolUseIDs(msg.Content)
			if len(toolUses) == 0 {
				continue
			}
			nextIdx := i + 1
			...
			if nextIdx < len(req.Messages) && req.Messages[nextIdx].Role == "user" {
				nextResults = anthropicToolResultIDs(req.Messages[nextIdx].Content)
			}

			keep := make(map[string]struct{}, len(toolUses))
			...
			if len(missing) > 0 {
				msg.Content = downgradeAnthropicToolUses(msg.Content, missing)
			}
			if len(keep) > 0 && nextIdx < len(req.Messages) && req.Messages[nextIdx].Role == "user" {
				expectedByUserIndex[nextIdx] = keep
			}

		case "user":
			results := anthropicToolResultIDs(msg.Content)
			if len(results) == 0 {
				continue
			}
			...
			if len(orphaned) > 0 {
				msg.Content = downgradeAnthropicToolResults(msg.Content, orphaned)
			}
		}
	}
}
...
func anthropicToolUseIDs(content json.RawMessage) map[string]struct{} {
...
func anthropicToolResultIDs(content json.RawMessage) map[string]struct{} {
	ids := make(map[string]struct{})
	var blocks []json.RawMessage
	if err := json.Unmarshal(content, &blocks); err != nil {
		return ids
...
func downgradeAnthropicToolUses(content json.RawMessage, ids map[string]struct{}) json.RawMessage {
	return rewriteAnthropicContentBlocks(content, func(raw json.RawMessage) (json.RawMessage, bool) {
		...
		return marshalAnthropicTextBlock(formatDowngradedToolUse(block)), true
	})
}

func downgradeAnthropicToolResults(content json.RawMessage, ids map[string]struct{}) json.RawMessage {
	return rewriteAnthropicContentBlocks(content, func(raw json.RawMessage) (json.RawMessage, bool) {
		...
		return marshalAnthropicTextBlock(formatDowngradedToolResult(block)), true
	})
}

func rewriteAnthropicContentBlocks(content json.RawMessage, rewrite func(json.RawMessage) (json.RawMessage, bool)) json.RawMessage {
	var blocks []json.RawMessage
	if err := json.Unmarshal(content, &blocks); err != nil {
		return content
	}
...
func marshalAnthropicTextBlock(text string) json.RawMessage {
	b, _ := json.Marshal(AnthropicTextBlock{Type: "text", Text: text})
	return b
}

func formatDowngradedToolUse(block AnthropicToolUseBlock) string {
	input := strings.TrimSpace(string(block.Input))
	if input == "" || input == "null" {
		input = "{}"
	}
	return fmt.Sprintf("[tool_use omitted: id=%s name=%s input=%s]", block.ID, block.Name, input)
}

func formatDowngradedToolResult(block AnthropicToolResultBlock) string {
	content := strings.TrimSpace(anthropicToolResultText(block.Content))
	if content == "" {
		content = "(empty)"
	}
	return fmt.Sprintf("[tool_result omitted: tool_use_id=%s content=%s]", block.ToolUseID, content)
}
...
func anthropicToolResultText(raw json.RawMessage) string {
	...
	for _, blockRaw := range blocks {
		...
		if err := json.Unmarshal(blockRaw, &typed); err != nil {
			continue
		}
		switch typed.Type {
		case "text":
		...
		case "tool_result":
			...
			if err := json.Unmarshal(raw, &tr); err == nil {
				toolResults = append(toolResults, openAIMessage{
					Role:       "tool",
					Content:    anthropicToolResultText(tr.Content),
