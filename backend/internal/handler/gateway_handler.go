...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			// 等待路径和利润门路径在终端准入后绑定；Copilot sticky
			// overflow 只服务当前请求，必须保留原有会话绑定。
			if shouldBindStickyAfterWait(selection) && (selection.ProfitGateActive() || !selection.Acquired) {
	...
	for {
		...
		for {
			...
			} else if selection.PreserveStickyBinding {
			...
			if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
				...
				if h.copilotGatewayService == nil {
					...
					if copilotResult != nil {
						...
						setActualUpstreamEndpoint(c, copilotUpstreamEndpoint(copilotResult))
			...
			// Copilot 非 200 响应已由专用转发器写回客户端，不得再进入
			// 通用成功/计费或 failover 路径。
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	if errors.Is(err, service.ErrGroupRPMExceeded) || errors.Is(err, service.ErrUserRPMExceeded) || errors.Is(err, service.ErrUserTPMExceeded) {
		msg := pkgerrors.Message(err)
