...
func (h *CopilotGatewayHandler) Messages(c *gin.Context) {
	...
	if !reqStream && isMaxTokensOneHaikuRequest(reqModel, reqMaxTokens) {
...
func (h *CopilotGatewayHandler) submitUsageRecordTask(parent context.Context, task service.UsageRecordTask) {
	if task == nil {
		return
	}
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			...
			).Error("copilot.usage_record_task_panic_recovered")
		}
	}()
	task(ctx)
}
