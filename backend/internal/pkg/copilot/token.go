...
func ExchangeToken(httpClient *http.Client, githubToken string) (*CopilotToken, error) {
	...
	refreshAfter := tokenResp.RefreshIn - 60
	if tokenResp.RefreshIn <= 0 {
		refreshAfter = int64(time.Until(expiresAt).Seconds()) - 60
	}
	if refreshAfter < 30 {
		refreshAfter = 30 // minimum 30 seconds
	}
	refreshAt := now.Add(time.Duration(refreshAfter) * time.Second)
