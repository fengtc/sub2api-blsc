...
const copilotBillingClass = computed(() => {
  ...
  if (!usage) return ''
  if (usage.gross_quantity >= copilotBillingCreditLimit) return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
  if (usage.gross_quantity >= copilotBillingCreditLimit * 0.8) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
  return 'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300'
})

const copilotBillingCreditLimit = 20000

const copilotBillingTooltip = computed(() => {
  const usage = props.account.copilot_billing_usage
  if (!usage) return ''
  return `GitHub Billing ${usage.period} ${usage.username}: gross ${formatCredits(usage.gross_quantity)} / ${formatCredits(copilotBillingCreditLimit)} AI credits, net $${formatCost(usage.net_amount)}`
})
</script>
