...
import type { UserAttributeValuesMap } from '@/types'
...
const form = reactive({ email: '', password: '', username: '', notes: '', role: 'user' as 'user' | 'admin', balance: '', concurrency: 1, rpm_limit: 0, tpm_limit: 0, customAttributes: {} as UserAttributeValuesMap })
...
const submit = async () => {
  ...
  loading.value = true
  try {
    const { balance: rawBalance, customAttributes, ...rest } = { ...form }
    ...
    const populatedAttributes = Object.fromEntries(
      Object.entries(customAttributes).filter(([, value]) => value.trim() !== '')
    ) as UserAttributeValuesMap
    if (Object.keys(populatedAttributes).length > 0) {
      await adminAPI.userAttributes.updateUserAttributeValues(createdUser.id, populatedAttributes)
    }
...
watch(() => props.show, (v) => { if(v) Object.assign(form, { email: '', password: '', username: '', notes: '', role: 'user', balance: '', concurrency: 1, rpm_limit: 0, tpm_limit: 0, customAttributes: {} }) })
